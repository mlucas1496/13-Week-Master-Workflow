...
# ---------------------------------------------------------------------------
# SSL fix: when launched as a .app bundle the terminal env vars are not
# inherited, so Python's `requests` falls back to certifi and fails against
# the Zscaler corporate SSL proxy.  Set the CA bundle early — before any
# network code is imported — so every requests.get() call picks it up.
# ---------------------------------------------------------------------------
_ZSCALER_CERT = (
    "/Library/Application Support/Bullish/Zscaler/Certs/zscaler-custom-cert-store.pem"
)
if os.path.exists(_ZSCALER_CERT):
    os.environ.setdefault("REQUESTS_CA_BUNDLE", _ZSCALER_CERT)
    os.environ.setdefault("SSL_CERT_FILE", _ZSCALER_CERT)
import uuid
import json
import time
import shutil
...
# ---------------------------------------------------------------------------
# Step 1: Serve the Weekly Balances Rollforward static build
# ---------------------------------------------------------------------------

STEP1_DIST_DIR = os.path.join(os.path.expanduser("~"), "13wcf-rollforward", "dist")


@app.route("/step1/")
def step1_app():
    return send_file(os.path.join(STEP1_DIST_DIR, "index.html"))


@app.route("/assets/<path:filename>")
def step1_assets(filename):
    return send_file(os.path.join(STEP1_DIST_DIR, "assets", filename))


@app.route("/boc-api/<path:boc_path>")
def boc_api_proxy(boc_path):
    """Proxy Bank of Canada Valet API (used by the rollforward app)."""
    import requests as req
    url = f"https://www.bankofcanada.ca/{boc_path}"
